...
def test_neg_occupation():
    ...
    with pytest.raises(RuntimeError) as excinfo:
        edges, occ = cdiff.floatEvolveTimeStep(
            occupation, beta=1, prevMinIndex=0, prevMaxIndex=2, N=5
        )
...
def test_min_greater_max():
    ...
    with pytest.raises(RuntimeError) as excinfo:
        edges, occ = cdiff.floatEvolveTimeStep(
            occupation, beta=1, smallCutoff=0, prevMinIndex=2, prevMaxIndex=0, N=20
        )
...
def test_max_greater_array_length():
    ...
    with pytest.raises(IndexError) as excinfo:
        edges, occ = cdiff.floatEvolveTimeStep(
            occupation, beta=1, prevMinIndex=0, prevMaxIndex=3, smallCutoff=0, N=30
        )
...
def test_neg_min_edge():
    ...
    with pytest.raises(TypeError) as excinfo:
        edges, occ = cdiff.floatEvolveTimeStep(
            occupation, beta=1, prevMinIndex=-1, prevMaxIndex=2, smallCutoff=0, N=30
        )
...
def test_single_occupation_filled():
    ...
    edges, occupied = cdiff.floatEvolveTimeStep(
        occupation, beta=1, prevMinIndex=0, prevMaxIndex=1, smallCutoff=5, N=10
    )
...
def test_smallCutoff_optional():
    ...
    occupation = np.array([10, 0, 0])
    edges, occ = cdiff.floatEvolveTimeStep(
        occupation, beta=1, prevMinIndex=0, prevMaxIndex=1, N=10
    )
...
def test_diffusion_constructor():
    ...
    if not d.getsmallCutoff() == 1:
        errors.append(
            f"Small cutoff should be initialized to 1 but is {d.getsmallCutoff()}"
        )
...
def test_diffusion_initializeOccupancyAndEdges():
    ...
    assert (
        num_of_elements
        == len(edges[0]) & num_of_elements
        == len(edges[1]) & num_of_elements
        == len(occ)
    )
...
def test_diffusion_iterateTimestepInplaceFalse():
    ...
    num_of_elements = 2
    assert (
        num_of_elements == len(edges[0])
        and num_of_elements == len(edges[1])
        and num_of_elements + 1 == len(occ)
    )  # I think occupancy gets pushback once in iterateTimestep and another time in floatEvolveTimeStep
...
def test_diffusion_iterateTimestepInplaceTrue():
    ...
    assert (
        ...
        and num_of_elements == len(minEdge)
        and num_of_elements == len(maxEdge)
    )
