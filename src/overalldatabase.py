...
if __name__ == '__main__':
    ...
    ax.set_yscale("log")
    ax.set_xlabel(r"$t / \ln(N)$")
    ax.set_ylabel(r"$\sigma^{2} (N, t) / \ln(N)^{2/3}$")
    quantiles = [2, 6, 20, 100, 300]
    cm = LinearSegmentedColormap.from_list('rg', ['tab:orange', 'tab:red', "tab:purple", 'tab:blue'], N=256)
    colors = [
        cm(1.0 * i / len(quantiles) / 1) for i in range(len(quantiles))
    ...
    ax.set_yscale("log")
    ax.set_xlabel(r"$t / \ln(N)$")
    ax.set_ylabel(r"$\overline{\Delta x}(N, t)$")
    ...
    ax.annotate("", xy=(8, 2*10**4), xytext=start_coord,
            arrowprops=dict(shrink=0., facecolor='gray', edgecolor='white', width=20, headwidth=50, headlength=30), zorder=0)
    ax.annotate(r"$N=10^{2}$", xy=(200, 4.5), c=colors[0], rotation=90-abs(theta), rotation_mode='anchor')
