...
ax.set_ylabel(r"$(\ln(N))^{-1/2} \left(\mathrm{Var}^{\mathrm{Num}}(\mathrm{Min}_L^N)-\mathrm{Var}^{\mathrm{Num}}(\mathrm{Sam}_L^N)\right)$")
...
for i, Nexp in enumerate(Ns):
    ...
    ax.plot(cdf_df['Distance'] / logN, var_theory / np.sqrt(logN), c=colors[i], ls='--', alpha=0.5)
    ...
    # Need to make sure that all points are positive
    assert np.all(env_var >= 0)

    ax.scatter(dist_new / logN, env_var / np.sqrt(logN), label=Nlabels[i], color=colors[i], s=10)
...
ax.set_xlim([0.5, 750])
fig.savefig("./Figures/EnvironmentalVarianceRecovery.pdf", bbox_inches='tight')