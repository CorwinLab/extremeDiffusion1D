...
cdf_dir = f"/home/jacob/Desktop/talapasMount/JacobData/CleanData/FPTCDFPaperFixed/{Nexp}"
...
ax.plot(max_df['Distance'] / logN, max_df['Variance'], label=r'$\mathrm{Var}(\mathrm{Min}_L^N)$', c=max_color, alpha=alpha)
...
ax.plot(cdf_df['Distance'] / logN, cdf_df['Sampling Variance'], label=r'$\mathrm{Var}(\mathrm{Sam}_L^N)$', c=gumbel_color, alpha=alpha)
...
ax.plot(cdf_df['Distance'] / logN, cdf_df['Env Variance'], label=r'$\mathrm{Var}(\mathrm{Env}_L^N)$', c=quantile_color, alpha=alpha)
...
ax.plot(einstein_df['Distance'] / logN, einstein_df['Variance'], label=r'$\mathrm{SSRW} \quad \mathrm{Var}(\mathrm{Min}_L^N)$', c=einstein_color, alpha=alpha)
...
ax.plot(xvals, xvals ** 4, c='k', ls='--')
ax.plot(xvals, xvals ** 3, c='k', ls='--')

ax.annotate(r'$L^3$', xy=(250, 10**6), rotation=25, fontsize=12)
ax.annotate(r'$L^4$', xy=(200, 10**10), rotation=30, fontsize=12)
...
fig.savefig("./Figures/CompleteVariance.pdf", bbox_inches='tight')