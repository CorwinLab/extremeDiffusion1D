...
def calculateMeanVar(files):
    mean = np.zeros(shape=(2856, 11))
    squared = np.zeros(shape=(2856, 11))
    count = 0
    for f in files: 
        print(f)
        data = np.loadtxt(f, skiprows=1, delimiter=',')
        if data.shape != (2856,11):
            continue
        mean+=data
        squared += data**2
...
dir = "/home/jacob/Desktop/talapasMount/JacobData/BetaCDFFixed/"
dirs = os.listdir(dir)
run_again = False
...
for beta in [0.01, 0.1, 1, 10]: 
    ...
    axv.plot(time / logN, var[:,3], label=beta)
