...
d.center = np.arange(0, allOcc.shape[0]) - d.time / 2
...
ax.hlines(500 + L/2, 0, 1000, color='g', ls='--')
ax.hlines(500 - L/2, 0, 1000, color='g', ls='--')
ax.scatter(fpt_time, fpt_position / 2 + 500, marker='x', c='r', zorder=2, s=100)

pad = 10
ax.annotate(r"$L$", (100, 500 + L/2 + pad/2), color='g')
ax.annotate(r"$-L$", (100, 500 - L / 2 - pad), color='g')
ax.annotate(r"$\mathrm{Min}^N_L$", (750, 500 + 75), color='r')

ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)

ax.set_ylabel(r"$\mathrm{Distance}$")
