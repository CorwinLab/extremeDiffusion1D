...
dir = '/home/jacob/Desktop/corwinLabMount/CleanData/ContinuousNormalized/M*.txt'
files = glob.glob(dir)
data_sum = None
...
smaller_time = np.loadtxt("../Continuous/Time.txt")
smaller_mean = np.loadtxt("../Continuous/Mean.txt")
smaller_var = np.loadtxt("../Continuous/Var.txt")

fig, ax = plt.subplots()
xvals = np.linspace(100, 1000)
ax.plot(xvals, 4*xvals ** (1/2), ls='--', label=r'$\sqrt{t}$', c='k')
ax.plot(time, mean, c='b', label=r'$\xi_1$')
ax.plot(smaller_time, smaller_mean, c='r', label=r'$\xi_2$')
ax.set_xlim([1, 10**4])
ax.set_xlabel("Time")
ax.set_xscale("log")
ax.set_yscale("log")
...
ax.plot(smaller_time, smaller_var, c='r', label=r'$\xi_2$')
ax.set_xlim([1, 10**4])
ax.set_xscale("log")
ax.set_yscale("log")
ax.set_xlabel("Time")
