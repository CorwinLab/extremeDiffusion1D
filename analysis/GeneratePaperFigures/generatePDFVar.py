...
for i in range(quantiles.shape[1]):
    ...
    if prettifyQuad(N) == '1e100':
        ax.plot(time / np.log(N), var / (np.log(N)**(2/3)), alpha=alpha, c=colors[1])
    elif prettifyQuad(N) == '1e300':
        ax.plot(time / np.log(N), var / (np.log(N)**(2/3)), alpha=alpha, c=colors[0])
...
for i in range(db.var.shape[1]):
    ...
    if N == np.quad("1e2"):
        ax.plot(db.time / logN, db.var[:, i] / logN**(2/3), alpha=alpha, c=colors[4])
    elif N == np.quad("1e11"):
        ax.plot(db.time / logN, db.var[:, i] / logN**(2/3), alpha=alpha, c=colors[3])
    elif N == np.quad("1e20"):
        ax.plot(db.time / logN, db.var[:, i] / logN**(2/3), alpha=alpha, c=colors[2])
