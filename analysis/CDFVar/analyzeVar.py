...
files = glob.glob("/home/jacob/Desktop/corwinLabMount/CleanData/CDFVar/Quartiles*.txt")
finished_files = []
nParticles = 1000000
...
quartile_var = (
    quartile_squared_sum / len(finished_files)
    - (quartile_sum / len(finished_files)) ** 2
)
...
ax.plot(
    time / np.log(nParticles), theoryVarLongTimes, label=th.NthQuartVarStrLargeTimes
)
ax.set_xscale("log")
ax.set_yscale("log")
