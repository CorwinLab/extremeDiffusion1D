...
def stackExchange(t, L, nmax=10):
    ...
    for k in np.arange(-nmax, nmax):
        phi = L * (1+2*k)
        sum += (-1)**abs(k) * phi / np.sqrt(2 * np.pi * t**3) * np.exp(-(phi)**2 / 2 / t)
    return sum

def jacob(t, L, nmax=10):
    sum = 0
    for k in np.arange(-nmax, nmax): 
        sum += np.sqrt(2) * L / np.sqrt(np.pi * t**3) * (-1)**(abs(k+1))*(k-1/2) * np.exp(-2*L**2/t * (k-1/2)**2)
    return sum

def stackExchange2(t, L, nmax=10):
    sum = 0
    for n in range(nmax): 
        sum += n * np.pi / L**2 * (1-(-1)**n) * np.sin(n * np.pi  / 2) * np.exp(-n**2 * np.pi**2 * t / (2 * L**2))
    return sum

def continousPDF(t, L, nmax=10, D=1/2):
    sum = 0
    for n in range(0, nmax): 
        sum += (-1)**(n+1) * L**2/((2*n+1)*np.pi * D) * np.exp(-(n+1/2)**2 * np.pi**2 * D * t / L**2)
...
ax[0][0].plot(np.arange(1, 500_000), stackExchange(np.arange(1, 500_000), maxPosition, nmax=1000))
...
ax[0][1].plot([meanN.astype(float), meanN.astype(float)], [0, max(N_particle_pdf.astype(float))], color='r')
