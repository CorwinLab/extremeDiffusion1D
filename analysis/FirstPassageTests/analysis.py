...
for i in range(maxTime):
    pdf.iterateTimeStep()
    pdf_arr = pdf.getPDF()
    passageProbability[i] = pdf_arr[1] * 1/2 + pdf_arr[-2] * 1/2
...
fig, ax = plt.subplots()
time = np.array(range(maxTime))[::2]
print(np.nansum(levyDistribution(time, maxPosition)))
ax.plot(time, passageProbability[::2], label='SSRW')
...
print(np.sum(passageProbability[::2]))
