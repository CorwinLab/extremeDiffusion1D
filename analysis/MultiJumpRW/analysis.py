...
dir = '/home/jacob/Desktop/talapasMount/JacobData/MultiJumpRW/'
...
samples = 100000
sum = np.zeros(step_size)
sum_squared = np.zeros(step_size)

for i in range(samples):
    rand_vals = randomDirichlet(step_size)
    sum += rand_vals
    sum_squared += rand_vals ** 2

sum /= samples 
sum_squared /= samples

print('Mean Rand Val', np.mean(sum), 1/11)

print(f"gamma={np.sum(sum_squared)}")
print(f"q0 = {np.sum(sum**2)}")

xvals = np.arange(-width, width + 1)

print(sigma)
print(np.sum(sum * xvals**2))